...
export default function Contact() {
  ...
  return (
    <div className="bg-white-50 dark:bg-neutral-900" id="contact">
        ...
        <div>
          ...
          <div className="flex justify-center mt-2 text-dark-600 dark:text-neutral-400">
            <ContactLink 
              ...
              qrSrc="/images/hero/telegram-qr.svg"
              qrAlt="QR Code Telegram"
              qrWidth={120}
              qrHeight={120}
              color="blue"
            />
            <ContactLink 
              ...
              qrSrc="/images/hero/whatsapp-qr.svg"
              qrAlt="QR Code WhatsApp"
              qrWidth={120}
              qrHeight={120}
              color="green"
            />
        ...
        <div>
          ...
          {!isSubmitSuccessful && (
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <input type="checkbox" id="" className="hidden" {...register('botcheck')} />
              ...
              <div className="mb-4 mt-4 relative">
                <label htmlFor="topic" className="sr-only">Тема сообщения</label>
                <div className="relative">
                  <select
                    ...
                    className={`w-full px-4 py-3 border-2 placeholder:text-neutral-800 dark:text-white rounded-md outline-none dark:placeholder:text-neutral-200 dark:bg-neutral-900 focus:ring-4 bg-white appearance-none ${
                      errors.topic ? 'border-rose-500 focus:border-rose-500 ring-rose-100 dark:border-rose-500 dark:focus:border-white dark:ring-0' : 'border-neutral-300 focus:border-neutral-600 ring-neutral-100 dark:border-neutral-600 dark:focus:border-white dark:ring-0'
                    }`}
                ...
                {errors.topic && <div className="mt-1 text-rose-500"><small>{errors.topic.message}</small></div>}
              </div>
              ...
              <div className="mb-4 mt-4">
                <textarea
                  ...
                  className={`w-full px-4 py-3 border-2 placeholder:text-neutral-400 dark:text-white dark:placeholder:text-neutral-500 dark:bg-neutral-900 rounded-md outline-none h-36 focus:ring-4 ${
                    errors.message ? 'border-rose-500 focus:border-rose-500 ring-rose-100 dark:ring-0' : 'border-neutral-300 focus:border-neutral-600 ring-neutral-100 dark:border-neutral-600 dark:focus:border-white dark:ring-0'
                  }`}
                  {...register('message', {
                    required: 'Необходимо ввести текст сообщения',
                  })}
                />
                {errors.message && <div className="mt-1 text-rose-500"><small>{errors.message.message}</small></div>}
              </div>
              ...
              <div className="flex items-center mb-4 mt-4">
                ...
                <div>
                  <span className="text-sm">
                    ...
                    <span onClick={handleModalToggle} className="text-red-600 hover:underline cursor-pointer">
          ...
          {isSubmitSuccessful && (
            <div className={`flex flex-col items-center justify-center text-center rounded-md ${isSuccess ? 'text-green-500' : 'text-rose-500'}`}>
              <svg width="100" height="100" className={isSuccess ? 'text-green-500' : 'text-rose-500'} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  d={isSuccess 
              ...
              <h3 className="py-5 text-2xl font-medium">{isSuccess ? 'Успешно!' : 'Ошибка'}</h3>
              <p className="text-neutral-900 dark:text-neutral-300 md:px-4">
                {isSuccess ? 'Ваше сообщение отправлено.' : 'Ваше сообщение не было отправлено. Пожалуйста, попробуйте снова позже.'}
              </p>
      ...
      <PrivacyPolicyModal isOpen={isModalOpen} onClose={handleModalToggle} />
    </div>
  );
}

function ContactLink({ platform, href, qrSrc, qrAlt, qrWidth, qrHeight, color }) {
  return (
    <div className="flex flex-col items-center space-y-2 mx-4">
      <div className="flex items-center space-x-2">
        <Icon platform={platform} color={color} />
        <a href={href} target="_blank" className={`hover:text-${color}-500`} style={{ width: '90px' }}>
          {platform}
        </a>
      </div>
      <div>
...
function FormInput({ type, placeholder, register, error, id }) {
  return (
    <div className="mb-4 mt-4 relative">
      <input
        type={type}
        ...
        className={`w-full px-4 py-3 border-2 placeholder:text-neutral-800 dark:text-white rounded-md outline-none dark:placeholder:text-neutral-200 dark:bg-neutral-900 focus:ring-4 bg-white ${
          error ? 'border-rose-500 focus:border-rose-500 ring-rose-100 dark:border-rose-500 dark:focus:border-white dark:ring-0' : 'border-neutral-300 focus:border-neutral-600 ring-neutral-100 dark:border-neutral-600 dark:focus:border-white dark:ring-0'
        }`}
        {...register}
      />
      {error && <div className="mt-1 text-rose-500"><small>{error.message}</small></div>}
    </div>
  );
}
