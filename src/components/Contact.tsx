  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <InputField 
        ...
        register={register('email', { required: 'Необходимо ввести Ваш Email', pattern: { value: /^\S+@\S+$/i, message: 'Введите, пожалуйста, правильный адрес электронной почты' } })}
        error={errors.email}
      />
      ...
      {isActivationOrInstallation && (
        <InputField
          id="orderNumber"
          placeholder="Номер заказа"
          register={register('orderNumber', { required: isActivationOrInstallation && 'Номер заказа обязателен' })}
...
const InputField: React.FC<{ id?: string, type?: string, placeholder: string, register: any, error: any }> = ({ id, type = 'text', placeholder, register, error }) => (
  <div className="mb-5">
    <input
      id={id}
      type={type}
...
const SelectField: React.FC<{ id: string, register: any, options: string[], error: any }> = ({ id, register, options, error }) => (
...
const TextAreaField: React.FC<{ placeholder: string, register: any, error: any }> = ({ placeholder, register, error }) => (
  <div className="mb-3">
    <textarea
      placeholder={placeholder}
...
const MessengerLink: React.FC<{ href: string, name: string, colorClass: string, icon: React.ReactNode }> = ({ href, name, colorClass, icon }) => (
  <div className="flex flex-col items-center space-y-2 mx-4">
    ...
    <div>
      <a href={href} target="_blank" rel="noopener noreferrer">
        <img
          src={`/images/hero/${name.toLowerCase()}-qr.svg`}
...
const ErrorMessage: React.FC<{ message: string }> = ({ message }) => (
  <div className="flex flex-col items-center justify-center text-center text-neutral-900 dark:text-neutral-300 rounded-md">
    ...
    <h3 className="py-5 text-2xl font-medium text-rose-500">Упс, что-то пошло не так!</h3>
