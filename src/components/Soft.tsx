...
export default function Soft() {
  ...
  const handleDownloadClick = (links: { link: string; label: string }[]) => {
    links.length === 1 ? window.open(links[0].link, "_blank", "noopener noreferrer") : setModalLinks(links);
  };
  ...
  return (
    <div className="bg-gray-50 dark:bg-neutral-900" id="soft">
        ...
        <div className="relative text-base font-semibold mt-6 bg-neutral-200 dark:bg-neutral-800 rounded-lg inline-flex flex-col sm:flex-row sm:flex-wrap justify-center sm:mt-8 p-1 gap-1">
          {DeviceTypes.map((type) => (
            <button
              ...
              className={`rounded-md py-2 px-4 whitespace-nowrap transition-colors duration-300 ease-in-out ${selectedType === type ? "bg-white dark:bg-neutral-600 text-neutral-900 dark:text-neutral-100" : "text-neutral-900 dark:text-neutral-400 hover:bg-white dark:hover:bg-neutral-700"}`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-16 grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-16">
        {products.filter(({ type }) => type === selectedType).map(({ title, mostPopular, description, features, downloadLinks, docs, docsLinks }) => {
          const displayedFeatures = features.length > 4 ? [...features.slice(0, 3), "и т.д."] : features;
          return (
            <div key={title} className={`rounded-lg py-8 relative flex flex-col ${mostPopular ? "border-red-300 border-2 border-solid dark:border-red-600" : "border-neutral-300 border dark:border-neutral-600"} hover:bg-neutral-100 dark:hover:bg-neutral-700 hover:shadow-lg transition-all duration-300`}>
              <h3 className="px-6 text-lg font-semibold line-clamp-1">{title}</h3>
              {mostPopular && <p className="mx-6 absolute top-0 px-4 py-1 -translate-y-1/2 bg-red-100 text-red-600 rounded-full text-sm font-semibold tracking-wide shadow-md">Топ продаж</p>}
              ...
              <div className="flex mt-4 mx-6">
                <button
                  ...
                  className="block px-6 py-3 font-medium leading-4 text-center rounded-lg bg-red-600 text-white shadow-md hover:bg-green-500 transition-colors duration-200 ease-in-out w-full"
                >
                  Скачать
                </button>
