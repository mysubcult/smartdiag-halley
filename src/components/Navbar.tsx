...
export default function Navbar() {
  ...
  const handleNavigationClick = useCallback(
    (anchor: string) => (event: React.MouseEvent) => {
      ...
      if (router.pathname !== "/") {
        router
          .push("/")
          .then(() => router.push(anchor, undefined, { scroll: false }));
      } else {
  ...
  const [isDesktopMenuOverflowing, setIsDesktopMenuOverflowing] =
    useState(false);
  const desktopMenuRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const handleResize = () => {
      if (isLgScreen && desktopMenuRef.current) {
        const { clientWidth, scrollWidth } = desktopMenuRef.current;
        setIsDesktopMenuOverflowing(scrollWidth > clientWidth);
      } else {
        setIsDesktopMenuOverflowing(false);
      }
    };
  ...
  return (
    <nav className="navbar fixed top-0 left-0 right-0 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-400 border-b border-neutral-200 dark:border-neutral-700 backdrop-blur-sm bg-white/90 dark:bg-neutral-900/80 z-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="relative flex h-16 items-center justify-between flex-wrap">
          <div className="flex flex-1 items-center justify-start">
            ...
            {isLgScreen && !isDesktopMenuOverflowing && (
              <div
                ref={desktopMenuRef}
                className="flex flex-nowrap items-center space-x-5 ml-4"
              >
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    ...
                    onClick={
                      item.anchor
                        ? handleNavigationClick(item.anchor)
                        : undefined
                    }
                  >
                    {item.name}
      ...
      {isMenuOpen && (!isLgScreen || isDesktopMenuOverflowing) && (
